...
type ITour = {
    ...
    operator: {
      id: number, email: string, createdAt: string, updatedAt: string, userId: number, companyName: string, 
      description: string, firstName: string, lastName: string, website: string, phone: string, status: string, 
      philosophy: string, photo: string, rejectionReason: string
    },
    country: {
      id: number, iso2: string, iso3: string, translations: [ { id: number, countryIso2: string, languageCode: string, name: string } ]
    },
    city: {
      id: number, countryIso2: string, translations: [ { id: number, cityId: number, languageCode: string, name: string } ]
    }
}
...
const getTourDetails = (tour: ITour) => [
    ...
    { label: "Дата початку", value: new Date(tour.startDate).toLocaleDateString() },
    { label: "Дата закінчення", value: new Date(tour.endDate).toLocaleDateString() },
...
function TourDetails() {
    const { result: tour, query: { isLoading } } = useShow<ITour>();
    if (isLoading) {
                       ...
                       <TableRow>
                        ...
                        <TableRow>
                            ...
                            <TableCell>
                                ...
                                {tour.photos && tour.photos.map((photo) => (
                                    <Image key={photo.id} src={photo.url} alt="Tour photo" width={200} height={200} style={{ objectFit: 'cover' }} />
                                ))}
